...
para("The result contains 725,714 distinct wind measurements and 757,260 deployment-associated rows. The first season contributes 531,937 rows and the second contributes 225,323.")
heading("Fields and direction conventions", 2)
para("The public wind table retains deployment_id, wind_sensor_id, timestamp, wind_speed, wind_gust and wind_direction. Wind speed and gust are in meters per second. Wind speed is the average and gust is the maximum speed over each one-minute recording interval. These measurements describe conditions near the logger, rather than sustained exposure at every butterfly position.")
...
para("Please also check the scientific definitions and limitations. The central points are that BI is an index, camera temperature is an approximate instrument reading, incomplete wind coverage has not been filled. Recorded device times and measurement definitions should be consistent across the README, dictionary and XML. Known recording gaps are documented rather than presented as complete continuous series.")
