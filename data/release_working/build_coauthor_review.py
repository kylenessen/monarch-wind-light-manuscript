...
table(["File", "Rows", "What one row represents"], [
    ("deployments.csv", f"{counts['deployments']:,}", "A camera deployment within a season"),
...
para("The photo index covers 56,066 first-season JPEGs and 167,821 reviewed second-season JPEGs. Use season together with deployment_id to join the observation tables to deployments. This matters because SC12 is used in both seasons. Add image_filename when joining classifications or temperatures to the photo index. The analysis tables omit season because they contain only first-season records.")
...
para("The public wind table retains season, deployment_id, wind_sensor_name, timestamp_recorded, wind_speed_m_s, wind_gust_m_s and wind_direction_degrees. Wind speed and gust are in meters per second. Gust is the logger maximum for the recording interval, normally one minute. These measurements describe conditions near the logger, rather than sustained exposure at every butterfly position.")
...
para("PS01 has wind observations through January 31, 2025 at 04:04, although its images continue later. SC12 has a regular wind sequence through December 23, 2024 at 12:51, followed by a 22-day gap and 69 zero-speed and zero-gust records on January 14, 2025. Those later SC12 records could reflect field or servicing conditions. We do not have enough evidence to label either endpoint a battery failure, and we have not deleted the later zeros on that assumption.")
...
para("Each classification contains the season, deployment identifier, camera name, image filename, recorded time and observer information. It preserves counts of image cells in the 0, 1–9, 10–99 and 100–999 butterfly categories. It also preserves counts of occupied cells marked as receiving direct sunlight in each occupied category. These primitives allow readers to calculate alternative category-weighted indices without changing the original annotations.")
...
para("The temperature table contains 56,066 first-season records linked to image filename, recorded time, season, deployment and camera. Temperatures were extracted from the image overlays with optical character recognition, then previously reviewed and corrected where necessary. The release copies the reviewed Celsius values, including missing values, without new recalibration or clock changes. The table omits OCR confidence and processing-status columns to keep the released measurements simple.")
...
para("Please review whether the release scope and file descriptions accurately represent the observations, including deployments outside the manuscript subset. Check the camera and wind-meter assignments, deployment boundaries and location descriptions. The season and deployment identifiers should let a reader connect every table and photograph without relying on local knowledge of the equipment names.")
...
para("The complete local staging package currently points to the reviewed photo directories through links, avoiding a second copy of the full archive. Before final upload, the indexed JPEGs must be placed in ordinary archives with the season and deployment paths preserved. Symbolic links and unlisted source files should not be distributed. Working reconciliation records, source provenance audits and this review guide are kept outside the public observational package.")
