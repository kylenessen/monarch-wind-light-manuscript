...
The main analysis reads [analysis_30_minute.csv](../data/release/analysis_30_minute.csv) and [analysis_next_day.csv](../data/release/analysis_next_day.csv) directly. Their field names use BI, delta BI and explicit weather units. They contain 17 and 10 columns respectively. Every column is used by a retained analysis, descriptive summary or the focused observer sensitivity. The signed square-root next-day response is calculated in R. The stored signed cube-root 30-minute response is preserved from the historical input.
...
The larger CSVs directly under data retain the historical inputs and original variable names. Rebuild the reduced public analysis inputs from those files without needing the portable photo drive with this command.
...
The focused BI category and observer sensitivity checks are a separate step. Their preparation script regenerates inputs under four category mappings, checks the lower-bound mapping against the historical inputs and exports each variant using the public analysis schema. The observer check reads the released 30-minute table directly. The variant files are generated analysis intermediates and are not added to the data release.
...
To regenerate all release tables, the photo index, dictionary and draft XML, mount MonarchSSD and run `uv run analysis/prepare_data_release.py`. The script reads the frozen reconciliation under `/Volumes/MonarchSSD/data_release/reconciled_2026-09-13`. It writes the public files to data/release and stages a matching package on the drive. Scripts and working audit notes are outside the public package. Read the [release notes](../data/release/README.md) before using these draft products.
