...
`thirty_minute_gamm_analysis.R` audits the 50 unique candidates from the earlier 30-minute analysis using maximum likelihood, then refits the selected model using restricted maximum likelihood. It preserves the original candidate identifiers after removing duplicate models M20 and M44. The script writes a complete legacy fit audit and model-selection outputs to `analysis/outputs/30_minute`. It regenerates model diagnostics and only generates a wind by direct-sun interaction surface when the selected model contains that tensor interaction.
...
`next_day_window_analysis.R` reconstructs and compares the 74 Next Day Window candidates that were defined in the original analysis. These are M1 through M72 and M77 through M78. It uses maximum likelihood and AICc for comparison, records every fit and convergence warning, and refits the selected model using restricted maximum likelihood. Outputs are written to `analysis/outputs/next_day_window`, including the complete legacy audit in `model_comparison_comprehensive.csv`.

`harmonized_model_comparison.R` is the primary candidate-model analysis for the 30-minute and Next Day response windows. It applies the same environmental hypothesis templates to the window-specific predictors. The 30-minute primary set includes previous BI and time since sunrise in every candidate. The Next Day primary set includes previous-day maximum BI and window duration. Separately ranked sensitivities omit previous BI, and the 30-minute analysis also includes a no-time sensitivity. The script uses AICc for both windows and writes complete comparisons, selected-model summaries, conditional wind effects, and the 30-minute prediction figure to `analysis/outputs/harmonized_model_comparison`.
