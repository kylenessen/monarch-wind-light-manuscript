...
Table~\ref{tab:30min_candidates} gives the fixed-effect structure of every primary 30-minute candidate. In this table, $C_{30}$ is previous BI plus minutes since the first daily observation, $W_{30}$ is maximum gust within the paired interval, $S_{30}$ is sun-exposed BI at the earlier image, and $T_{30}$ is mean camera temperature between the paired images. Smooth terms used shrinkage thin-plate regression splines with $k=5$. Every candidate also had the random-effect and correlation structure described above.
