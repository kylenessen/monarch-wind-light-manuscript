...
Wind direction was not analyzed because the historical prediction evaluated here concerns disruption as a function of wind-speed magnitude rather than direction. Directional exposure at the butterflies also could not be inferred from a pole heading without measurements of the intervening canopy and its effects on airflow. Surrounding canopy structure and its influence on local airflow were not quantified. We therefore interpret the logger values as nearby measurements at the pole position, not as demonstrated wind exposure at every butterfly position.
...
Under the monitored conditions, we did not find evidence that a single wind threshold consistently predicted declines in visible cluster size. Replicated work across seasons, sites, grove structures, cluster densities, and geographic regions, with within-canopy wind and direct physiological measurements, is needed to determine whether this result extends beyond the monitored setting.
