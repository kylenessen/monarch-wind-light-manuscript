...
To test whether wind was associated with short-term changes in visible cluster size, we compared 17 primary candidate models. All candidates produced rankable fits. The exact formula of M16 remained the best-supported model. It included previous BI as a control covariate and a linear three-way interaction among maximum wind gust, average temperature, and sun-exposed BI. M16 received nearly all candidate-set support (AICc = 8054.14, Akaike weight $>$ 0.9999). The model containing all three linear two-way interactions ranked second and was 34.82 AICc units higher. The same environmental three-way interaction ranked first when previous BI was omitted and when time within day was added to every candidate. The time-adjusted version had an AICc of 8052.02.
...
To test whether wind was associated with changes in visible cluster size over a longer response window, we compared 35 primary candidate models. All candidates produced rankable fits. The exact formula of M32 remained the best-supported model. It related Next Day Window change in daily maximum BI to previous-day maximum BI, window duration, and a tensor smooth interaction between maximum wind gust and cumulative sun-exposed BI. M32 ranked first (AICc = 650.01, Akaike weight = 0.368). A maximum-temperature-only model was also competitive ($\Delta$AICc = 1.89, Akaike weight = 0.143). The closest three-way mirror of M16 used temperature at the previous day's maximum BI and ranked third ($\Delta$AICc = 2.99, Akaike weight = 0.082). The control-only model ranked fifth ($\Delta$AICc = 3.58, Akaike weight = 0.061). M32 remained first in the sensitivity analysis without previous BI, although a three-way interaction using maximum temperature was within one AICc unit ($\Delta$AICc = 0.81). We refitted M32 using restricted maximum likelihood for estimation.
...
The Next Day Window analysis also identified a conditional association between maximum gust and sun-exposed BI. Its fitted response varied across combinations of maximum gust and cumulative sun-exposed BI rather than showing a uniformly negative wind relationship. This analysis contained 96 consecutive-day comparisons from one grove and had greater model-selection uncertainty than the 30-minute analysis. Three-way mirrors of the 30-minute hypothesis were evaluated using each temperature variable defined in the original Next Day analysis. The closest mirror, using temperature at the previous day's maximum BI, ranked third but was about three AICc units above M32. M32 remained first without adjustment for previous BI, although the maximum-temperature three-way model was then within one AICc unit. We therefore treat the selected Next Day relationship as complementary evidence that the relationship with wind was conditional, not as an independent replication of the specific three-way interaction found over 30 minutes.
...
\section{Harmonized Candidate Model Strategy}

The retained analyses used a common set of hypothesis templates while preserving the environmental variables originally defined for each response window. Let $C$ denote the required adjustment covariates, $W$ maximum gust, $S$ sun-exposed BI, and $T$ one temperature measure. Linear interactions followed marginality. Thus, a three-way interaction included all associated main effects and two-way interactions. The exact formulas of legacy models M16 and M32 were retained even where M32 differed from this marginality convention. A hierarchical tensor counterpart to M32 was also included.

\begin{table}[H]
\caption{Environmental predictors and adjustment covariates in the harmonized comparisons.}
...
\midrule
$C$ & Previous BI & Previous-day maximum BI and window duration \\
...
\begin{table}[H]
\caption{Shared hypothesis templates. Each temperature-dependent template was fitted once in the 30-minute analysis and separately for each of the three original Next Day temperature predictors.}
...
H05 & M32 tensor interaction & $C + ti(W,S)$ \\
...
T07 & M16 linear three-way interaction & $C + W * T * S$ \\
...
The primary 30-minute comparison contained the eight H templates and nine T templates, for 17 candidates. The control set $C$ was previous BI. Legacy M16 was candidate T07. M32 was mirrored as H05. The no-previous-BI sensitivity removed previous BI from all 17 candidates. The time-adjusted sensitivity added linear time within day to all 17 candidates. Each model included random intercepts for deployment and deployment-day and AR(1) correlation within deployment-day. All 51 fits completed without warnings. M16 ranked first in all three frameworks.

\subsection{Next Day Candidate Mapping}
\label{app:nextday_models}

The primary Next Day comparison contained the eight H templates and nine T templates repeated for each of the three original temperature predictors, for 35 candidates. The control set $C$ was previous-day maximum BI and window duration. Legacy M32 was candidate H05. M16 was mirrored by T07 using minimum temperature, maximum temperature, and temperature at the previous day's maximum BI in separate candidates. The no-previous-BI sensitivity removed previous-day maximum BI from all 35 candidates but retained window duration. Each model included a deployment random intercept and AR(1) correlation within deployment. All 70 fits completed without warnings. M32 ranked first in both frameworks.

All 121 harmonized fits were compared using maximum likelihood and AICc. Akaike weights were calculated separately within each response window and sensitivity framework. The selected formulas were refitted using restricted maximum likelihood. Complete machine-readable formulas, fit outcomes, rankings, and selected-model summaries accompany the analysis code.

\section{Legacy 30-Minute Candidate Audit}
\label{app:30min_legacy}

For transparency, this section documents the 52 named candidates from the earlier 30-minute analysis. M20 duplicated M17, and M44 duplicated M41. These duplicates were removed from the legacy audit, leaving 50 unique candidates. The harmonized comparison reported in the main text supersedes this candidate set.
...
All 50 unique legacy candidates produced rankable fits. M16 was selected by AIC and refitted using restricted maximum likelihood. M16 included previous BI and the linear three-way interaction among maximum wind gust, average temperature, and sun-exposed BI. The same exact formula was carried forward into the harmonized comparison.


\section{Legacy Next Day Candidate Audit}
\label{app:nextday_legacy}

For transparency, this section documents the 74 candidate formulas defined in the earlier Next Day analysis. These were M1 through M72 and M77 through M78. Model identifiers M73 through M76 were not defined. The harmonized comparison reported in the main text supersedes this candidate set.
...
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{1.6cm}>{\raggedright\arraybackslash}p{\dimexpr\linewidth-1.6cm-2\tabcolsep\relax}@{}}
\caption{Complete list of 74 formulas in the legacy Next Day candidate audit.} \label{tab:nextday_candidates} \\
\toprule
...
Of the 74 legacy candidates, 67 produced rankable fits. Seven fits with convergence warnings were excluded from ranking. M32 was selected by AICc and refitted using restricted maximum likelihood. The same exact formula was carried forward into the harmonized comparison.
