...
The Next Day set contained 75 candidate models. These included baseline-only models and single, additive, interactive, and smooth environmental effects. The set included a linear three-way interaction among maximum gust, mean window temperature, and cumulative sun-exposed BI, with previous-day maximum BI and window duration as adjustments. Next Day models included a deployment random intercept and an AR(1) residual correlation structure within deployment series. Because all qualifying pairs came from Spring Canyon, this random effect accounts for repeated observations among deployments within one grove and does not provide grove-level replication.
...
To test whether wind was associated with changes in visible cluster size over a longer response window, we compared 75 candidate models. The best-supported model, M32, related Next Day Window change in daily maximum BI to previous-day maximum BI, window duration, and a tensor smooth interaction between maximum wind gust and cumulative sun-exposed BI. Of the 75 candidates, 68 produced rankable fits. The remaining seven generated convergence warnings and were excluded from the ranking and Akaike-weight calculation. M32 ranked first (AICc = 650.01, Akaike weight = 0.613). The control-only model with no environmental predictors, M2, ranked second ($\Delta$AICc = 3.58, Akaike weight = 0.102). The linear three-way analogue of the selected 30-minute model, M79, ranked 24th (AICc = 662.26, $\Delta$AICc = 12.25, Akaike weight = 0.0013). We refitted M32 using restricted maximum likelihood for estimation.
...
This appendix documents the 75 candidate formulas compared in the Next Day Window analysis. The original analysis defined candidates M1 through M72 and M77 through M78. Model identifiers M73 through M76 were not defined. M79 was added as a prespecified linear day-scale analogue of the selected 30-minute three-way interaction.
...
\texttt{temp\_mean}: Mean temperature over the Next Day Window ($^{\circ}$C);
...
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{1.6cm}>{\raggedright\arraybackslash}p{\dimexpr\linewidth-1.6cm-2\tabcolsep\relax}@{}}
\caption{Complete list of 75 candidate formulas compared in the Next Day Window analysis.} \label{tab:nextday_candidates} \\
\toprule
...
M79     & Linear three-way interaction: wind\_max\_gust $\times$ temp\_mean $\times$ sum\_butterflies\_direct\_sun \\
\bottomrule
\end{longtable}

Of the 75 candidates, 68 produced rankable fits. Seven fits with convergence warnings were excluded from ranking. M32 was selected by AICc and refitted using restricted maximum likelihood. M79 ranked 24th with $\Delta$AICc = 12.25. Complete formulas, fit outcomes, warnings, and rankings are included with the reproducibility materials.
