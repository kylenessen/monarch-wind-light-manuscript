...
We then identified that the two response-window candidate sets had been developed at different times and did not express a common model-building strategy. We therefore replaced the reported comparisons with a harmonized set of hypothesis templates while retaining only the environmental variables defined in the original analyses. The primary analyses include previous BI as a linear adjustment in every model. Window duration is also required in every Next Day model. Separately ranked sensitivity analyses remove previous BI, and a further 30-minute sensitivity includes time within day in every model. The exact M16 and M32 formulas were retained, and each environmental hypothesis was mirrored as closely as the different sampling windows allowed. Both windows were ranked by AICc after maximum-likelihood fitting.

All 17 primary 30-minute candidates converged. M16 remained first with an AICc of 8054.14 and an Akaike weight greater than 0.9999. It also remained first without previous BI and with time within day included. All 35 primary Next Day candidates converged. M32 remained first with an AICc of 650.01 and an Akaike weight of 0.368. A maximum-temperature-only model was within two AICc units. The closest three-way mirror of M16 used temperature at the previous day's maximum BI and ranked third with a delta AICc of 2.99. M32 also remained first without previous BI, although a maximum-temperature three-way interaction was within one AICc unit. The revised manuscript reports the common model-building strategy, the separate control sensitivities, all fit outcomes, and the greater model-selection uncertainty in the Next Day analysis. Complete formulas and rankings are provided with the reproducibility materials.
]
