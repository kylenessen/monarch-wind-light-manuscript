...
#locations([Original decision letter])
#response[
We have begun a substantial compression of the statistical presentation. We removed the simple linear regressions and their figure, the threshold-duration analysis and candidate-model appendix, the fixed 24-hour sensitivity analysis, and the temporal-window diagram. The retained manuscript now focuses on the immediate 30-minute and biologically aligned Next Day Window analyses. We are also reducing the introduction and recasting unmeasured physiology as a future research hypothesis. We will report the final page, figure, and table counts in the submitted version.
]
...
#response[
Draft in progress. We will document any available validation and clarify the measurement limitation. We will also state that humidity, vapor pressure deficit, precipitation, and surface wetness were not monitored and that the observational design does not support attributing all cluster changes specifically to wind.
]
...
#response[
Draft in progress. We will report the available Butterfly Index validation, inter-observer information, and sensitivity analyses. We will also state how image displacement and visibility during high-wind periods were assessed, or identify that limitation directly where an assessment was not possible.
]
...
#response[
Draft in progress. We will define the deployment as the monitoring and time-series unit rather than presume biological independence among all deployments. We will also revise “site fidelity” terminology unless grove-level persistence was directly measured.
]
...
#response[
Thank you for identifying the duplicated candidates. We removed exact duplicate 30-minute candidates M20 and M44 without renumbering the remaining candidates. We then repeated the retained candidate-model comparisons using maximum likelihood, followed by restricted maximum likelihood refits of the selected models for coefficient estimation and figures. The corrected 30-minute comparison contains 50 unique candidates, of which 49 produced rankable fits. It selected M16 rather than the previously reported M50. We also reconstructed the Next Day Window set from the 74 formulas actually defined in the original analysis. Sixty-seven of 74 candidates produced rankable fits and seven warning fits were excluded. M32 remained selected, but its corrected Akaike weight was 0.614. We will moderate the support language and report the convergence accounting, updated rankings, and the change in the 30-minute selected model in the revised Methods, Results, tables, and response locations.
]
