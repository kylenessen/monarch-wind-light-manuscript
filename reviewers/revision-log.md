...
**Decision.** Replace the reported comparisons with a shared hypothesis-template strategy and retain the window-specific environmental predictors. Do not use mean overnight temperature in the Next Day analysis. Include previous BI and time since sunrise in every primary 30-minute candidate. Include previous-day maximum BI and duration in every primary Next Day candidate. Rank sensitivities that omit previous BI and time separately. Compare all candidates using maximum likelihood and AICc, then refit selected formulas using restricted maximum likelihood. Retain the earlier candidate audits in the reproducibility records rather than the manuscript appendix.

**Analysis result.** The 30-minute primary comparison contained 17 candidates, all of which converged. The full wind by temperature by sun-exposed BI interaction ranked first with AICc 8052.02 and an Akaike weight greater than 0.9999. The same environmental three-way interaction ranked first when previous BI was omitted and when time since sunrise was omitted. The Next Day primary comparison contained 35 candidates, all of which converged. The centered wind by sun-exposed BI tensor interaction ranked first with AICc 650.01 and an Akaike weight of 0.368. A maximum-temperature-only model was within two AICc units. The three-way model using temperature at the previous day's maximum BI ranked third with $\Delta$AICc = 2.99. Without previous BI, the centered tensor remained first and a maximum-temperature three-way model was within one AICc unit. Across the primary and sensitivity frameworks, all 121 attempted fits completed without convergence warnings.
...
**Draft response.** We removed the duplicate candidates and used a common set of environmental hypothesis templates in both response windows. Previous BI and time since sunrise are required in every primary 30-minute model. Previous-day maximum BI and duration are required in every primary Next Day model. Adjustment variables are removed only in separately ranked sensitivities. We used only the window-specific environmental predictors, and we excluded mean overnight temperature from the Next Day candidates. The same random structures were used wherever the sampling hierarchy permitted. Observer was not included because it was confounded with deployment. The selected environmental structures were stable across the adjustment sensitivities, and all candidate fits converged.

**Verification.** The shared comparison script completed successfully. It fitted 17 candidates in each of three 30-minute frameworks and 35 candidates in each of two Next Day frameworks. All 121 fits converged. The earlier audits remain available in the reproducibility records.
